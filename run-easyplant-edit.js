...
async function main() {
  ...
  const replacementPlant = {
    name: 'Anthurium Red flowering plant',
    path: './data/uploads/plants/anthurium-red.jpg'
  };
  ...
  // Save masked preview
  const maskedPath = path.join(OUTPUT_DIR, 'easyplant-masked.png');
  fs.writeFileSync(maskedPath, maskedBuffer);
  console.log(`   ✅ Saved: ${maskedPath}`);
  
  // Copy original as "before"
  const beforePath = path.join(OUTPUT_DIR, 'easyplant-BEFORE.jpg');
  ...
  if (result.success && result.data?.images?.length > 0) {
    ...
    for (let i = 0; i < result.data.images.length; i++) {
      ...
      try {
        ...
        const outputPath = path.join(OUTPUT_DIR, `easyplant-AFTER-${i + 1}.png`);
